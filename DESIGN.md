...
- 제품 디렉터리에 공개한 앱은 모두 `운영 중`으로 표시하고, 베타·웹 데모 같은 접근 방식은 링크나 설명에서 별도로 안내한다.
- 모션은 `opacity`와 `transform/translate` 중심으로 만들고 `prefers-reduced-motion`에서 정지한다.
- 3D는 최대 2–4도의 포인터 반응형 2.5D로 제한하며 정보 탐색을 늦추지 않는다.

### 최상위 링크 허브

- 첫 화면부터 실제 제품으로 이동할 수 있도록 짧은 제작자 프로필과 직접 링크 목록을 나란히 둔다. 모바일에서는 작은 프로필 아래 단일 열 링크로 전환한다.
- 카드의 주 동작은 등록된 웹 서비스 또는 App Store로 이동한다. 외부 실행 링크가 없는 제품은 상세 소개로 연결하며, 운영 여부와 설치 링크의 등록 여부를 혼동하지 않는다.
- 웹 데모는 링크 자체에 명시하고, 외부 링크가 있는 카드의 제품 소개는 별도 보조 링크로 유지한다.
- 실제 앱 아이콘과 기존 다국어 소개 문구를 재사용한다. 소개를 읽기 위한 페이지는 `/apps/`와 제품별 하위 경로에서 계속 제공한다.
- 꽃의 피어남·포인터·터치·스크롤 반응을 작은 프로필 비주얼에 유지하되, 링크를 등장 애니메이션 뒤에 숨기지 않는다.
